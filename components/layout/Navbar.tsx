      ...
      <div className="mx-auto max-w-7xl">
        <nav aria-label="Main Navigation" className="relative inset-0 z-30 flex w-full grid-cols-[auto_1fr_auto] md:grid">
          ...
          <div className="flex gap-x-4">
            ...
            <button
              className="flex items-center justify-center rounded-lg hover:bg-secondary/20 md:hidden"
              ...
              {navItems.map((item, index) => (
                <li key={index} className="flex items-center">
                  <Link
                    href={item.href}
                    className={cn(
                      "flex min-h-10 items-center whitespace-nowrap rounded-lg px-3 py-2 font-medium transition-all hover:text-foreground hover:bg-secondary/20",
                      pathname === item.href
          ...
          <div className="relative flex flex-1 items-center justify-end md:h-12">
            ...
            <a
              href={getPhoneLink()}
              className="hidden items-center gap-2 text-sm text-muted-foreground hover:text-primary md:flex mr-4"
            >
              ...
              <div className="ml-2 hidden md:block">
                <div className="flex items-center justify-end">
                  <a
                    ...
                    className="group relative flex min-h-10 items-center justify-center rounded-lg bg-primary px-3 py-2 font-medium text-primary-foreground transition-all ease-in-out hover:rounded-xl hover:bg-primary-hover"
                  >
          ...
          <ul className="flex flex-col gap-y-2 px-4 py-6">
            {navItems.map((item, index) => (
              <li key={index} className="flex items-center">
                <Link
                  href={item.href}
                  className="flex min-h-10 w-full items-center whitespace-nowrap rounded-lg px-3 py-2 font-medium text-muted-foreground transition-all hover:bg-secondary/20 hover:text-foreground"
